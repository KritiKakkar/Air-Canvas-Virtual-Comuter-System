...
for i, penSize in enumerate(range(5,25,5)):
    pens.append(ColorRect(1000,50+100*i,100,100, (50,50,50), str(penSize)))

penBtn = ColorRect(1000, 0, 100, 50, color, 'Pen')
...
penBtn = ColorRect(1000, 0, 100, 50, color, 'Pen')
shapeBtn = ColorRect(1100, 0, 100, 50, color, 'Shapes')
#############shapes#################
shapes = []
for i, shapeSize in enumerate(range(1,4,1)):
    shapes.append(ColorRect(1100,50+100*i,100,100, (50,50,50), str(shapeSize)))



# white board button
boardBtn = ColorRect(0, 0, 100, 100, (255,255,0), 'Board')

#Rectangle
RecBtn = ColorRect(100, 0, 100, 100, (255,255,0), 'Rec')
...
while True:
    ...
    if upFingers:
        ...
        if upFingers[1] and not whiteBoard.isOver(x, y):
            ...
            if not hideShapeSizes:
                for shape in shapes:
                    if shape.isOver(x, y):
                        shapeselected = int(shape.text)
                        shape.alpha = 0
                    else:
                        shape.alpha = 0.5
            ...
            # shapes button
            if shapeBtn.isOver(x, y) and not coolingCounter:
                coolingCounter = 10
                shapeBtn.alpha = 0
                hideShapeSizes = False if hideShapeSizes else True
                shapeBtn.text = 'Shape' if hideShapeSizes else 'Hide'
            else:
                shapeBtn.alpha = 0.5
    ...
    # put Rectangle buttin
    RecBtn.drawRect(frame)
    cv2.rectangle(frame, (RecBtn.x, RecBtn.y), (RecBtn.x +RecBtn.w, RecBtn.y+RecBtn.h), (255,255,255), 2)
    ...
    shapeBtn.color = color
    shapeBtn.drawRect(frame)
    cv2.rectangle(frame, (shapeBtn.x, shapeBtn.y), (shapeBtn.x +shapeBtn.w, shapeBtn.y+shapeBtn.h), (255,255,255), 2)
    if not hideShapeSizes:
        for shape in shapes:
            shape.drawRect(frame)
            cv2.rectangle(frame, (shape.x, shape.y), (shape.x +shape.w, shape.y+shape.h), (255,255,255), 2)
